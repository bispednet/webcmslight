...
<body class="<?= htmlspecialchars($bodyClass, ENT_QUOTES, 'UTF-8'); ?>">
    ...
    <!-- WhatsApp concierge -->
    <div class="concierge" data-concierge>
      <section class="concierge-panel" data-concierge-panel hidden aria-label="WhatsApp concierge">
        <header class="concierge-header">
          <div><strong>bisp&amp;d concierge</strong><span><?= $isEnglish ? 'Tell us what you need' : 'Prepariamo la richiesta insieme' ?></span></div>
          <button type="button" data-concierge-close aria-label="Chiudi">&times;</button>
        </header>
        <div class="concierge-messages" data-concierge-messages></div>
        <form class="concierge-form">
          <input type="text" maxlength="600" required autocomplete="off" placeholder="<?= $isEnglish ? 'Write your answer...' : 'Scrivi la tua risposta...' ?>">
          <button type="submit"><?= $isEnglish ? 'Send' : 'Invia' ?></button>
        </form>
        <a class="concierge-handoff" data-concierge-handoff hidden target="_blank" rel="noopener"><?= $isEnglish ? 'Continue on WhatsApp' : 'Continua su WhatsApp' ?></a>
      </section>
      <button type="button" class="whatsapp-fab" data-concierge-open aria-label="<?= $isEnglish ? 'Open WhatsApp concierge' : 'Apri il concierge WhatsApp' ?>">
    ...
    <script>
      window.BISPED_CONCIERGE = <?= json_encode([
          'csrf' => $conciergeCsrf,
          'locale' => $isEnglish ? 'en' : 'it',
          'phone' => '393346582116',
          'greeting' => $isEnglish ? 'Hi, I can prepare your request for our team. What can we help you with?' : 'Ciao, preparo la richiesta per il nostro team. Di cosa hai bisogno?',
