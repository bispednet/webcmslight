...
$tagline      = $settings['site_tagline']     ?? ($en ? 'IT · Telecom · Service · Piombino' : 'Informatica · Telefonia · Assistenza · Piombino');
$heroTitle    = $settings['hero_title_home']  ?? ($en ? 'Technology chosen well.<br>Installed better.' : 'Tecnologia scelta bene.<br>Installata meglio.');
$heroSubtitle = $settings['hero_subtitle_home'] ?? ($en
    ? 'Bisped is the tech reference point in Piombino: PCs, smartphones, gaming, connectivity, energy and technical support with real people.'
    : 'Bisped è il punto di riferimento tech a Piombino: PC, smartphone, gaming, connettività, energia e assistenza tecnica con persone vere.');
...
// URL prefix for internal links (English routes live under /en)
$p = $en ? '/en' : '';
    ...
    <div class="grid gap-10 lg:grid-cols-[1fr_420px] lg:items-center">
        <div>
            ...
            <div class="mt-8 flex flex-wrap gap-3">
                <a href="<?= $p ?>/products" class="btn-primary">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-4 h-4">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z"/>
                    </svg>
                    <?= $en ? 'Browse catalog' : 'Vedi il catalogo' ?>
                </a>
                <a href="<?= $p ?>/contatti?topic=<?= rawurlencode($en ? 'Technical support' : 'Assistenza tecnica') ?>" class="btn-outline">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M11.42 15.17 17.25 21A2.652 2.652 0 0 0 21 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 1 1-3.586-3.586l6.837-5.63m5.108-.233c.55-.164 1.163-.188 1.743-.14a4.5 4.5 0 0 0 4.486-6.336l-3.276 3.277a3.004 3.004 0 0 1-2.25-2.25l3.276-3.276a4.5 4.5 0 0 0-6.336 4.486c.091 1.076-.071 2.264-.904 2.95l-.102.085m-1.745 1.437L5.909 7.5H4.5L2.25 3.75l1.5-1.5L7.5 4.5v1.409l4.26 4.26m-1.745 1.437 1.745-1.437m6.615 8.206L15.75 15.75M4.867 19.125h.008v.008h-.008v-.008Z"/>
                    </svg>
                    <?= $en ? 'Request support' : 'Richiedi assistenza' ?>
                </a>
            </div>
            ...
            <div class="mt-10 grid grid-cols-3 gap-3 max-w-sm">
                <div class="text-center p-3 border rounded-md" style="border-color:var(--c-border);background:var(--c-surface)">
                    ...
                    <div class="text-xs font-bold uppercase tracking-widest text-muted mt-1"><?= $en ? 'Years' : 'Anni' ?></div>
                </div>
                <div class="text-center p-3 border rounded-md" style="border-color:var(--c-border);background:var(--c-surface)">
                    <div class="font-display text-2xl font-black" style="color:var(--bisped-red)">Lab</div>
                    <div class="text-xs font-bold uppercase tracking-widest text-muted mt-1"><?= $en ? 'Repairs' : 'Riparazioni' ?></div>
                </div>
                <div class="text-center p-3 border rounded-md" style="border-color:var(--c-border);background:var(--c-surface)">
                    <div class="font-display text-2xl font-black" style="color:var(--bisped-red)">500+</div>
                    <div class="text-xs font-bold uppercase tracking-widest text-muted mt-1"><?= $en ? 'Products' : 'Prodotti' ?></div>
                </div>
            </div>
        </div>
        ...
        <div class="relative" <?= AdminMode::dataAttrs('settings', 'hero_image_home', null, 'image') ?>>
            ...
            <img src="<?= htmlspecialchars($heroImage, ENT_QUOTES, 'UTF-8') ?>"
                 alt="<?= $en ? 'Bisped store in Piombino' : 'Negozio Bisped a Piombino' ?>"
...
<section class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 py-2" data-animate>
    <!-- Gaming -->
    <a href="<?= $p ?>/products?cat=gaming" class="promo-banner" style="aspect-ratio:16/10">
    ...
    <!-- Smartphone -->
    <a href="<?= $p ?>/products?cat=smartphone" class="promo-banner" style="aspect-ratio:16/10">
    ...
    <!-- Notebook / Lavoro -->
    <a href="<?= $p ?>/products?cat=notebook" class="promo-banner" style="aspect-ratio:16/10">
    ...
    <div class="container mx-auto max-w-7xl px-4 lg:px-6">
        <div class="flex flex-wrap items-center justify-center gap-x-10 gap-y-3 text-xs font-bold uppercase tracking-wider text-muted">
            <span class="flex items-center gap-2">
                ...
                <?= $en ? 'Official warranty' : 'Garanzia ufficiale' ?>
            </span>
            <span class="flex items-center gap-2">
                ...
                <?= $en ? 'Shipping available' : 'Spedizione disponibile' ?>
            </span>
            <span class="flex items-center gap-2">
                ...
                <?= $en ? 'Local tech support' : 'Assistenza tecnica locale' ?>
            </span>
            <span class="flex items-center gap-2">
                ...
                <?= $en ? 'Free consultation' : 'Consulenza gratuita' ?>
            </span>
            <span class="flex items-center gap-2">
                ...
                <?= $en ? 'Remote support' : 'Teleassistenza remota' ?>
            </span>
        </div>
    </div>
</section>
...
<!-- ─── BRAND PARTNER BAR ────────────────────────────────────────────────────── -->
<section class="brand-bar -mx-4 md:-mx-6 lg:-mx-0" aria-label="<?= $en ? 'Partners and brands' : 'Partner e brand' ?>">
    <div class="container mx-auto max-w-7xl px-4 lg:px-6">
...
<section class="py-4" data-animate>
    <div class="mb-8">
        <div class="section-label mb-4"><?= $en ? 'Departments' : 'Reparti' ?></div>
        <h2 class="font-display text-3xl font-black md:text-4xl" style="color:var(--c-acc)"><?= $en ? 'Find the right counter right away.' : 'Trova subito il banco giusto.' ?></h2>
        <p class="mt-3 text-muted max-w-2xl"><?= $en ? 'Every department leads to something useful: a product to evaluate, a service to request, a person to talk to.' : 'Ogni reparto porta a una scelta utile: un prodotto da valutare, un servizio da richiedere, una persona con cui parlare.' ?></p>
    </div>
...
<section class="py-4" data-animate>
    <div class="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
            <div class="section-label mb-4"><?= $en ? 'Featured' : 'In evidenza' ?></div>
            <h2 class="font-display text-3xl font-black md:text-4xl" style="color:var(--c-acc)"><?= $en ? 'Store products,<br>with workshop-grade advice.' : 'Prodotti da negozio,<br>con consulenza da laboratorio.' ?></h2>
        </div>
        <a href="<?= $p ?>/products" class="text-sm font-black uppercase tracking-widest" style="color:var(--bisped-red)">
            <?= $en ? 'Full catalog →' : 'Tutto il catalogo →' ?>
        </a>
    </div>
    <div class="grid gap-4 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4">
        <?php foreach ($featured as $index => $product): ?>
            <a href="<?= $p ?>/products/<?= htmlspecialchars($product['slug'], ENT_QUOTES, 'UTF-8') ?>"
...
<section data-animate>
    <a href="<?= $p ?>/servizi#assistenza" class="promo-banner block" style="aspect-ratio:21/5">
        ...
        <div class="promo-banner__overlay" style="background:linear-gradient(135deg,rgba(209,25,32,.6) 0%,rgba(0,0,0,.15) 60%)">
            <span class="promo-banner__tag"><?= $en ? 'Active service' : 'Servizio attivo' ?></span>
            <p class="promo-banner__title"><?= $en ? 'Remote support —<br>we help you from home too' : 'Teleassistenza remota —<br>ti aiutiamo anche da casa' ?></p>
            <span class="promo-banner__cta"><?= $en ? 'Request support →' : 'Richiedi supporto →' ?></span>
        </div>
    </a>
</section>
...
<section data-animate>
    <div class="mb-8">
        <div class="section-label mb-4"><?= $en ? 'Happy customers' : 'Clienti soddisfatti' ?></div>
        <h2 class="font-display text-3xl font-black md:text-4xl" style="color:var(--c-acc)"><?= $en ? 'Our satisfied customers.' : 'I nostri clienti soddisfatti.' ?></h2>
        <p class="mt-3 text-muted max-w-2xl"><?= $en ? 'What people who stopped by the counter, called or wrote to us have to say.' : 'Quello che dicono le persone che sono passate dal banco, hanno chiamato o ci hanno scritto.' ?></p>
    </div>
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <div class="review-card" data-animate data-animate-delay="0">
            ...
            <p class="review-card__text"><?= $en
                ? '"Precise hardware, great looks. The gaming PC they built me works perfectly and the advice was excellent."'
                : '"Precisione hardware, bella estetica. Il pc gaming che mi hanno assemblato funziona benissimo e la consulenza è stata eccellente."' ?></p>
            <div class="review-card__author">Marco R.</div>
            <div class="review-card__source"><?= $en ? 'Store customer — Custom gaming build' : 'Cliente negozio — Gaming build custom' ?></div>
        </div>
        <div class="review-card" data-animate data-animate-delay="80">
            ...
            <p class="review-card__text"><?= $en
                ? '"I had a hard drive with years of family photos. Data recovery completed in 48h. I had given up hope, I\'m thrilled."'
                : '"Avevo un hard disk con anni di foto di famiglia. Recupero dati completato in 48h. Non ci speravo più, sono contentissima."' ?></p>
            <div class="review-card__author">Silvia M.</div>
            <div class="review-card__source"><?= $en ? 'Data recovery — Piombino' : 'Recupero dati — Piombino' ?></div>
        </div>
        <div class="review-card" data-animate data-animate-delay="160">
            ...
            <p class="review-card__text"><?= $en
                ? '"Full management of our company network and TIM Business lines. Professional, punctual and always available."'
                : '"Gestione completa della nostra rete aziendale e delle linee TIM Business. Professionali, puntuali e sempre disponibili."' ?></p>
            <div class="review-card__author">Antonio B.</div>
            <div class="review-card__source"><?= $en ? 'Business solution — Local company' : 'Soluzione business — Azienda locale' ?></div>
        </div>
    </div>
</section>
...
<section data-animate>
    <div class="grid gap-10 lg:grid-cols-[1fr_440px] lg:items-start">
        <div>
            <div class="section-label mb-4"><?= $en ? 'Frequently asked questions' : 'Domande frequenti' ?></div>
            <h2 class="font-display text-3xl font-black md:text-4xl mb-2" style="color:var(--c-acc)"><?= $en ? 'We work with the best in the field.' : 'Lavoriamo con i migliori in campo.' ?></h2>
            <p class="text-muted text-sm leading-6 mb-6"><?= $en
                ? 'The best IT and telecom technologies at your disposal: TIM, WindTre, Vodafone, Fastweb, Enel, Heracomm, MSI, ASUS, Intel, AMD, Samsung, Huawei, Philips, HP, Canon.'
                : 'Le migliori tecnologie informatiche e di telecomunicazione a vostra disposizione: TIM, WindTre, Vodafone, Fastweb, Enel, Heracomm, MSI, ASUS, Intel, AMD, Samsung, Huawei, Philips, HP, Canon.' ?></p>
            <a href="<?= $p ?>/faq" class="btn-outline btn-sm"><?= $en ? 'All FAQs →' : 'Tutte le FAQ →' ?></a>
        </div>
        <div class="space-y-3">
            <details class="faq-item">
                <summary><?= $en ? 'Which payments do you accept?' : 'Quali pagamenti accettate?' ?></summary>
                <p><?= $en
                    ? 'We accept cash, debit and credit cards and bank transfers. For business purchases, deferred payments with invoice can be arranged.'
                    : 'Accettiamo contanti, bancomat, carte di credito e bonifico bancario. Per acquisti aziendali è possibile concordare pagamenti differiti con fattura.' ?></p>
            </details>
            <details class="faq-item">
                <summary><?= $en ? 'How does the warranty work?' : 'Come funziona la garanzia?' ?></summary>
                <p><?= $en
                    ? 'All products are sold with the official manufacturer warranty (minimum 24 months for consumers). For devices repaired in our workshop we offer a warranty on the work performed.'
                    : 'Tutti i prodotti sono venduti con garanzia ufficiale del produttore (minimo 24 mesi per i privati). Per i dispositivi riparati in laboratorio offriamo garanzia sul lavoro eseguito.' ?></p>
            </details>
            <details class="faq-item">
                <summary><?= $en ? 'Do you ship?' : 'Fate spedizioni?' ?></summary>
                <p><?= $en
                    ? 'Yes, we ship nationwide. Contact us for a quote or to check immediate availability before ordering online.'
                    : 'Sì, spediamo su tutto il territorio nazionale. Contattateci per un preventivo o per verificare la disponibilità immediata prima di ordinare online.' ?></p>
            </details>
            <details class="faq-item">
                <summary><?= $en ? 'How much does technical support cost?' : 'Quanto costa un\'assistenza tecnica?' ?></summary>
                <p><?= $en
                    ? 'Diagnosis is always free. The repair quote is given before proceeding, no surprises. We refuse to repair when it is not worth it.'
                    : 'La diagnosi è sempre gratuita. Il preventivo per la riparazione viene comunicato prima di procedere, senza sorprese. Ci rifiutiamo di riparare se non ne vale la pena.' ?></p>
            </details>
        </div>
    </div>
</section>
...
<section data-animate>
    <div class="info-card" style="border-color:rgba(209,25,32,.2)">
        <div class="grid gap-8 md:grid-cols-[1fr_1fr] md:items-center">
            <div>
                <p class="section-label mb-3"><?= $en ? 'Bisped newsletter' : 'Newsletter Bisped' ?></p>
                <h2 class="font-display text-2xl font-black" style="color:var(--c-acc)"><?= $en ? 'Tech news straight from Piombino' : 'Novità tech direttamente da Piombino' ?></h2>
                <p class="mt-3 text-sm" style="color:var(--c-muted)">
                    <?= $en
                        ? 'New 2025-2026 products, early-bird offers, buying guides and news from the tech world. No spam: max 2 emails per month.'
                        : 'Nuovi prodotti 2025-2026, offerte in anteprima, guide all\'acquisto e notizie dal mondo tech. Senza spam: massimo 2 email al mese.' ?>
                </p>
                <p class="mt-2 text-xs" style="color:var(--c-muted)"><?= $en ? 'By subscribing you accept our' : 'Iscrivendoti accetti la nostra' ?> <a href="<?= $p ?>/legal" style="color:var(--bisped-red)">privacy policy</a>. <?= $en ? 'Unsubscribe in one click.' : 'Cancellazione in un clic.' ?></p>
            </div>
            <form method="post" action="<?= $p ?>/contatti" class="space-y-3">
                <input type="hidden" name="topic" value="newsletter">
                <input type="hidden" name="message" value="Iscrizione newsletter bisped.net">
                <div>
                    <label class="form-label"><?= $en ? 'Your name' : 'Il tuo nome' ?></label>
                ...
                <button type="submit" class="btn-primary w-full"><?= $en ? 'Subscribe free →' : 'Iscriviti gratis →' ?></button>
            </form>
        </div>
    </div>
</section>
...
<section class="cta-strip" data-animate>
    <div class="grid gap-6 md:grid-cols-[1fr_auto] md:items-center">
        <div>
            <h2 class="font-display text-2xl font-black md:text-3xl" style="color:var(--c-acc)"><?= $en ? "Not sure what to choose? Even better." : 'Non sai cosa scegliere? Meglio così.' ?></h2>
            <p class="mt-2 text-muted max-w-xl"><?= $en
                ? 'Helping you buy once, well: the right product, a clean setup, support when you need it. That is the value of Bisped.'
                : 'Aiutarti a comprare una volta sola, bene: prodotto giusto, configurazione pulita, assistenza quando serve. Questo è il valore di Bisped.' ?></p>
        </div>
        <div class="flex flex-wrap gap-3">
            <a href="<?= $p ?>/contatti" class="btn-primary"><?= $en ? 'Talk to us' : 'Parla con noi' ?></a>
            <a href="<?= $p ?>/servizi" class="btn-outline"><?= $en ? 'Our services' : 'I nostri servizi' ?></a>
        </div>
    </div>
</section>
