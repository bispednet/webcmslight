...
<section class="space-y-6 max-w-4xl">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-muted">Compose reusable legal sections. Basic HTML is supported.</p>
        </div>
        <a href="/admin/legal" class="text-sm text-muted hover:text-acc transition">Back to list</a>
    </div>
    ...
    <form method="post" action="<?= htmlspecialchars($formAction, ENT_QUOTES, 'UTF-8'); ?>" class="space-y-5">
        ...
        <label class="text-sm text-muted flex flex-col gap-2">
            ...
            <input type="text" name="title" required
                   value="<?= htmlspecialchars($section['title'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
                   class="bg-bg2 border border-stroke rounded-md px-3 py-2 text-acc focus:border-cy focus:outline-none">
        </label>

        <label class="text-sm text-muted flex flex-col gap-2">
            <span>Content HTML</span>
            <textarea name="content_html" rows="10" required
                      class="bg-bg2 border border-stroke rounded-md px-3 py-2 text-acc focus:border-cy focus:outline-none font-mono text-xs leading-relaxed"><?= htmlspecialchars($section['content_html'] ?? '', ENT_QUOTES, 'UTF-8'); ?></textarea>
            <span class="text-xs text-muted">Allowed tags: p, h2-h4, strong, em, ul/ol/li, blockquote, links.</span>
        </label>

        <label class="text-sm text-muted flex flex-col gap-2 w-full md:w-40">
            <span>Sort Order</span>
            <input type="number" name="sort_order" min="0"
                   value="<?= htmlspecialchars((string)($section['sort_order'] ?? 0), ENT_QUOTES, 'UTF-8'); ?>"
                   class="bg-bg2 border border-stroke rounded-md px-3 py-2 text-acc focus:border-cy focus:outline-none">
        </label>
        ...
        <div class="flex items-center gap-3 pt-4">
            ...
            <a href="/admin/legal" class="text-sm text-muted hover:text-acc transition">Cancel</a>
        </div>
    </form>
</section>
